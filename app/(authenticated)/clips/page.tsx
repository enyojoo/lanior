...
export default function ClipsPage() {
  ...
  // Initialize local comments when current clip changes
  useEffect(() => {
    setLocalComments(currentClip.commentsList || [])
    setIsCommentsOpen(false)
    setProgress(0)
    setVideoLoaded(false)
  }, [currentClipIndex, currentClip.commentsList])

  // Prevent body scrolling when on clips page
  useEffect(() => {
  ...
  // Update the useEffect that handles video playback when the current clip changes
  useEffect(() => {
    // Reset states when changing clips
    setIsPlaying(false)
    setVideoLoaded(false)
    setProgress(0)
    ...
    return () => {
      ...
      // Ensure we stop any ongoing playback when changing clips
      if (video) {
        video.pause()
        video.currentTime = 0
      }
  ...
  const goToNextClip = () => {
    ...
    setTimeout(() => {
      setCurrentClipIndex((prevIndex) => (prevIndex === clipsData.length - 1 ? 0 : prevIndex + 1))
  ...
  const goToPreviousClip = () => {
    ...
    // Wait for fade out animation to complete
    setTimeout(() => {
      setCurrentClipIndex((prevIndex) => (prevIndex === 0 ? clipsData.length - 1 : prevIndex - 1))
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Don't navigate clips if comments are open or during transition
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      // If comments are open or during transition, don't allow clip scrolling
      ...
      <div className="relative flex justify-center items-center">
          ...
          <div className="relative h-full w-full rounded-lg overflow-hidden" onClick={togglePlay}>
            ...
            <div className="absolute bottom-6 left-4 right-16 z-10">
              <div className="flex flex-col space-y-2">
                ...
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center flex-1 min-w-0 pr-2">
                    <Avatar className="h-10 w-10 border-2 border-primary overflow-hidden flex-shrink-0">
                      <AvatarImage
                        src={currentClip.expert.avatar}
                        alt={currentClip.expert.name}
                        className="object-cover"
                      />
                      <AvatarFallback>{currentClip.expert.name[0]}</AvatarFallback>
                    </Avatar>
                    <div className="ml-2 overflow-hidden">
                      <div className="flex items-center gap-1">
                        <p className="text-white text-sm font-medium whitespace-nowrap overflow-hidden text-ellipsis">
                          {currentClip.expert.name}
                        </p>
                        {currentClip.expert.verified && (
                          <Badge
                            variant="outline"
                ...
                {currentClip.description && <p className="text-white/90 text-xs">{currentClip.description}</p>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="absolute right-[-60px] top-1/2 -translate-y-1/2 flex flex-col items-center gap-6 z-20">
          ...
          >
            ...
            <span className="text-primary dark:text-white text-xs mt-1">{currentClip.likes}</span>
          </button>
          ...
          <button className="flex flex-col items-center justify-center" onClick={toggleComments}>
            ...
            <span className="text-primary dark:text-white text-xs mt-1">{currentClip.comments}</span>
          </button>
          ...
          <div className="mt-6 flex flex-col items-center gap-4">
            <button
              ...
              onClick={(e) => {
                e.stopPropagation()
                if (!isTransitioning) goToPreviousClip()
              }}
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation()
                if (!isTransitioning) goToNextClip()
              }}
              disabled={isTransitioning}
            >
